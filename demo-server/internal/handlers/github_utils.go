...
func makeGithubLoginUrl() (string, error) {
	params := url.Values{}
	params.Add("client_id", utils.ENV["GITHUB_CLIENT_ID"])
	params.Add("redirect_uri", utils.ENV["GITHUB_CALLBACK_URL"])
...
func getGithubAccessToken(githubCode string) (GithubTokenResp, error) {
	...
	data := url.Values{}
	data.Set("client_id", utils.ENV["GITHUB_CLIENT_ID"])
	data.Set("client_secret", utils.ENV["GITHUB_CLIENT_SECRET"])
	data.Set("code", githubCode)
	data.Set("redirect_uri", utils.ENV["GITHUB_CALLBACK_URL"])
